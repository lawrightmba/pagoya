/**
 * Paula message template seed — idempotent, versioned, deploy-path ready.
 *
 * Run via:  pnpm --filter @workspace/api-server run seed:paula
 *
 * Contract:
 *  - Upsert on trigger_type: safe to run on every deploy.
 *  - Active values are EXPLICIT per-row — they match the intended prod state.
 *  - readiness_hard and readiness_hard_step2 are active=false until a live
 *    partner_programs row exists and the handoff flow is verified end-to-end.
 *  - Running twice in a row is a no-op (updated_at changes, content won't).
 *  - Add updated_at / content_sid / template_category columns before running
 *    (see B1 migration).
 *
 * Why two rows stay inactive (active=false):
 *  - readiness_hard:      contains "Paula encontró algo que podría interesarte"
 *                         — implies a live partner handoff. No partner yet.
 *  - readiness_hard_step2: microcrédito partner flow not yet wired.
 *  Activate both only after: (1) live partner_programs row confirmed,
 *  (2) handoff intercept in whatsapp-agent.ts tested against that partner's
 *  webhook, and (3) confirmed with product owner.
 */
...
export const PAULA_MESSAGES_EXPECTED_TOTAL  = 24;
...
const ROWS: SeedRow[] = [
  ...
  // ── Achievement ─────────────────────────────────────────────────────────────
  { active: true, trigger_type: "first_payment", cooldown_days: 9999, template_en: null,
  ...
  { active: true, trigger_type: "streak_5", cooldown_days: 9999, template_en: null,
  ...
  { active: true, trigger_type: "pti_cross_40", cooldown_days: 9999, template_en: null,
  ...
  { active: true, trigger_type: "pti_cross_60", cooldown_days: 9999, template_en: null,
  ...
  { active: true, trigger_type: "pti_cross_80", cooldown_days: 9999, template_en: null,
  ...
  { active: true, trigger_type: "milestone_90d", cooldown_days: 9999, template_en: null,
  ...
  // ── Recovery ─────────────────────────────────────────────────────────────────
  { active: true, trigger_type: "late_payment_1", cooldown_days: 14, template_en: null,
  ...
  { active: true, trigger_type: "pti_drop_7d", cooldown_days: 7, template_en: null,
  ...
  { active: true, trigger_type: "stalled_14d", cooldown_days: 7, template_en: null,
  ...
  { active: true, trigger_type: "pattern_late_2x", cooldown_days: 14, template_en: null,
  ...
  // ── Educational modules ───────────────────────────────────────────────────────
  { active: true, trigger_type: "module_unlock_1", cooldown_days: 9999, template_en: null,
    template_es:
`{{nombre}}, acabas de hacer algo más importante de lo que parece.
  ...
  { active: true, trigger_type: "module_unlock_2", cooldown_days: 9999, template_en: null,
    template_es:
`{{nombre}}, {{pti_score}} puntos. Momento de contarte algo que muy poca gente sabe con claridad.
  ...
  { active: true, trigger_type: "module_unlock_3", cooldown_days: 9999, template_en: null,
    template_es:
`{{nombre}}, hay un mito que le ha costado mucho dinero a mucha gente en México, y quiero que tú no seas una de esas personas:
  ...
  { active: true, trigger_type: "module_unlock_4", cooldown_days: 9999, template_en: null,
    template_es:
`{{nombre}}, PTI {{pti_score}}. Nivel Plata. Esto es lo que necesitas saber ahora.
  ...
  { active: true, trigger_type: "module_unlock_5", cooldown_days: 9999, template_en: null,
    template_es:
`{{nombre}}, llegaste al último módulo. Esto es lo que necesitas saber antes de solicitar tu primer crédito formal — para que nadie te sorprenda con letra chica.
  ...
  // ── Readiness gate ────────────────────────────────────────────────────────────
  // active=true: progress-only copy, no partner promise
  { active: true, trigger_type: "readiness_approaching", cooldown_days: 14, template_en: null,
    template_es:
...
Sigue así — cada pago es un paso más hacia tu primer crédito formal.` },
  ...
  // active=false: "Paula encontró algo que podría interesarte" implies a live partner handoff.
  // DO NOT activate until: (1) live partner_programs row exists,
  // (2) handoff intercept in whatsapp-agent.ts tested against partner webhook,
  // (3) confirmed with product owner.
  { active: false, trigger_type: "readiness_hard", cooldown_days: 9999, template_en: null,
    template_es:
`🏆 {{nombre}}, llegaste.
  ...
  // ── Gap report ────────────────────────────────────────────────────────────────
  { active: true, trigger_type: "not_yet_gap_report", cooldown_days: 30, template_en: null,
    template_es:
`{{nombre}}, llevas {{streak_days}} días construyendo tu historial financiero desde cero. Eso no es poco.
  ...
  { active: true, trigger_type: "winback_30d", cooldown_days: 9999, template_en: null,
  ...
  { active: true, trigger_type: "free_credit_nudge", cooldown_days: 7, template_en: null,
    template_es:
`💳 {{nombre}}, tienes {{free_bill_credits}} pago(s) gratis esperándote.
  ...
  // ── Profile enrichment questions (deferred follow-ups queued after Module 1) ──
  { active: true, trigger_type: "remittance_profile", cooldown_days: 9999,
    template_es:
...
`💸 *Quick profile question:*
  ...
  *2* — No
  ...
  { active: true, trigger_type: "employment_profile", cooldown_days: 9999,
    template_es:
`📋 *Una pregunta para tu perfil financiero:*
...
`📋 *Quick profile question:*
  ...
  *5* — Prefer not to say
  ...
  { active: true, trigger_type: "address_tenure", cooldown_days: 9999,
    template_es:
...
`🏠 *Last profile question:*
  ...
  *3* — More than 2 years
  ...
  // ── Intentionally inactive — partner-dependent ────────────────────────────────
  // This row exists to prevent future seed runs from re-inserting it as active.
  // Activate only after: (1) live partner_programs row confirmed, (2) handoff
  // intercept in whatsapp-agent.ts tested against that partner's webhook,
  // and (3) readiness_hard above also activated.
  { active: false, trigger_type: "readiness_hard_step2", cooldown_days: 0, template_en: null,
    template_es: "[DRAFT — microcrédito partner flow not wired. Do not activate without completing the lending partner handoff integration.]" },
];
...
export async function seedPaulaMessages(dbClient: typeof DbType): Promise<{
  ...
  for (const row of ROWS) {
    await dbClient.execute(drizzleSql`
