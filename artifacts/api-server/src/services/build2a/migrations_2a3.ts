...
export async function ensureBuild2a3Tables(): Promise<void> {
  ...
  await db.execute(sql`
    ...
    BEGIN
      ...
      -- SENTINEL GUARD A: if the constraint already contains a Package 2A-5+ code,
      ...
      -- SENTINEL GUARD B: if there is NO constraint (dropped by a later migration),
      -- check whether existing rows would violate this package's narrower constraint.
      -- If any row has a code outside this package's set, skip — the 2A-6 migration
      -- (or a later one) will reinstall the full cumulative constraint.
