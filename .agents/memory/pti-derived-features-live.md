...
- Real latent bug found (unfixed as of July 2026): missions.ts distinct_billers goal queries COUNT(DISTINCT biller_name) FROM bill_payments, but that table has service_name (biller_name is on profiles). Error swallowed as non-fatal → distinct_billers missions never progress in prod. One-word fix pending user approval.
