# Replit Agent Prompts — PTI Signal Expansion (v4.2 → v4.3) — REV 9
### Corrects Part E's false premise (categoria was already correctly captured). No code changed yet.
...
6. **Part E's premise corrected — there was no bug to fix.** Direct code verification found all four `bill_payments` insert paths (lines 267, 332, 447, 1051) already set `categoria` correctly (`service.category` or `"manual"`) on a `NOT NULL` column with no default. The earlier "~0% population" reading was a false signal caused by production `bill_payments` having 0 rows, not by the column going unwritten. Part E is downgraded from "fix" to a lightweight regression tripwire test, with an explicit instruction not to touch the already-correct insert-path code — building a fix against a false premise risks introducing a regression into code that currently works.
...
Part E — categoria regression guard (NOT a fix — see correction below):
13. CORRECTION TO THIS PART'S ORIGINAL PREMISE: prior drafts of this prompt assumed categoria was uncaptured at insert time, based on a ~0% population reading in production. That reading is a red herring — bill_payments has 0 rows in production, so 0% population is explained entirely by the table being empty, not by the column going unwritten. Direct code verification confirms all four insert paths (billpay.ts lines 267, 332, 447, 1051) already set categoria: service.category or categoria: "manual", and the column is text NOT NULL with no default — there is no code path that could insert a row without it. Do NOT modify any insert-path code for categoria; there is nothing to fix, and touching already-correct insert logic risks introducing a regression into code that currently works.
14. Instead, add a single lightweight regression test asserting that every one of the four bill_payments insert paths sets a non-empty categoria value (a static/source-level check, or an integration test exercising each path with representative inputs — whichever fits the existing test conventions in this codebase). This exists purely as a tripwire against a future edit accidentally breaking the existing correct behavior; it changes no application logic.

GUARDRAILS: no touching derivedSignals.ts, weights, or fair-lending code. Do NOT add or modify anything in services/fairLendingAdjustment.ts (non-test) under any circumstance, even if a grep pass suggests it "should" have a snapshot reference — it should not, by design. Do NOT modify bill_payments insert-path code for categoria (see Part E correction) — that logic is already correct. All isolation tests pass. All existing tests pass.

DEFINITION OF DONE: v4.2 scores unchanged (proven by test, including the case where all new optional fields are entirely absent from the input); status filter applied once and verified via synthetic fallido/solicitud_manual rows; new fields declared optional; DERIVED_FEATURE_DEFAULTS exists as the single source of truth and all 9 files/~12 sites reference it rather than duplicating default values; the two cast sites updated by hand with a flagging comment; the schema-completeness test asserts exact key-set equality (not a soft check) at all ten construction sites, with REQUIRED_SNAPSHOT_DEFAULTS individually and explicitly asserted; services/fairLendingAdjustment.ts (non-test) confirmed untouched and still zero-references PTIDataSnapshot; cureTimeMedianHours explicitly not built with reasoning documented; categoria regression test added with NO changes to insert-path code (Part E is a tripwire, not a fix); suite green.
```
...
- **Part E was almost a fix for a bug that didn't exist.** The "categoria unpopulated" finding traced back to an empty production table, not unwritten code — all four insert paths were already correct. This is worth remembering as a category of risk distinct from everything else this plan has caught: not a silent corruption from missing logic, but wasted (or actively harmful) work from acting on a plausible-but-wrong diagnosis. The fix was to downgrade to a regression tripwire and explicitly forbid touching the insert paths, rather than let a "fix" get built against already-correct code.
- **Ready for handoff.** Run Prompt 1 as specified above (REV 8/9 is now the authoritative version — REV 4 through REV 7's Prompt 1/2 bodies are superseded), then Prompt 1.5 per REV 7 unchanged, then Prompt 2 as specified above.
